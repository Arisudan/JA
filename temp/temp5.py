#!/usr/bin/env python3
"""
backend.py - Final: Predictive Decay for Smooth Low-Speed Graph
"""
import asyncio
import websockets
import json
...
def process_pulse():
    ...
    with pulse_lock:
        ...
        if delta_time > 0.05: # Debounce 50ms
            if last_pulse_time != 0:
                speed_mps = WHEEL_CIRCUMFERENCE / delta_time
                last_raw_speed = speed_mps * 3.6
            last_pulse_time = current_time
            # print(f"Pulse! Raw: {last_raw_speed:.1f}")
...
def calculate_physics_speed():
    """
    Uses predictive decay to prevent flat-lining at low speeds.
    """
    ...
    # 1. Predictive Decay:
    # If we haven't seen a pulse in a while, the max possible speed IS DROPPING.
    # We shouldn't hold the old high speed.
    if time_since_pulse > 0.1:
        max_possible_speed = (WHEEL_CIRCUMFERENCE / time_since_pulse) * 3.6
        if last_raw_speed > max_possible_speed:
            last_raw_speed = max_possible_speed # Force it down smoothly

    # 2. Hard Stop
    if time_since_pulse > TIMEOUT_SECONDS:
        last_raw_speed = 0.0
        
    # 3. Smooth Approach (Inertia)
    # Instead of snapping to last_raw_speed, move towards it by 10% per frame
    # This removes the "staircase" effect completely.
...
async def loop():
    while True:
        # Use the new PHYSICS Calculation
        ...
        # High speed loop (20Hz) for smooth physics
