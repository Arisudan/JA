#!/usr/bin/env python3
"""
backend.py - Final Version: Smoothing + 1s Logging + Timestamps
"""
import asyncio
import websockets
import json
...
def start_new_log():
    ...
    last_logged_int_sec = -1 # Reset 1-second counter
    ...
    try:
        ...
        # Initialize buffer for download
...
def log_data_point(t, tgt, act, up, lo, viol):
    ...
    # ONLY log if we entered a new integer second (0, 1, 2...)
    ...
    with pulse_lock:
        ...
        if delta_time > 0.01: 
            if last_pulse_time != 0:
                ...
                # SMOOTHING: 85% Old Speed + 15% New Speed
                # This prevents the "sharp edge" look
                current_speed_kmh = (current_speed_kmh * 0.85) + (new_speed * 0.15)
...
async def loop():
    while True:
        ...
        if test_state['running']:
            ...
            # Log only once per second (handled inside log_data_point)
        ...
        await asyncio.sleep(0.1) # Update UI smoothly at 10Hz
