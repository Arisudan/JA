#!/usr/bin/env python3
"""
backend.py - Final Version: Guaranteed Memory Logging for Instant Download
"""
import asyncio
import websockets
import json
...
def setup_logging():
    if not os.path.exists(LOG_DIR):
        try:
            os.makedirs(LOG_DIR)
            # Fix permissions so non-sudo users can read logs too
...
def start_new_log():
    ...
    # 1. Initialize Memory Buffer (Guarantees Download works)
...
def log_data_point(t, tgt, act, up, lo, viol):
    ...
    if current_int_sec > last_logged_int_sec:
        ...
        # 2. Save to Disk (If file is open)
    ...
    with pulse_lock:
        ...
        if delta_time > 0.01: 
            if last_pulse_time != 0:
                ...
                # Smooth transition (85% old, 15% new)
    ...
    try:
        ...
        async for msg in ws:
            ...
            if cmd == 'start':
                ...
                # Send the memory buffer directly
