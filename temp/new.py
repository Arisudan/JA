#!/usr/bin/env python3
"""
backend.py - Final Fixed Version for Hero Drive Cycle
"""
import asyncio
import websockets
import json
...
def start_new_log():
    ...
    try:
        ...
        current_csv_writer.writerow(header.split(","))
        current_log_file.flush()
        print(f"[LOG] Started logging to {filename}")
    except Exception as e:
        print(f"[LOG] Disk Error (RAM Download will still work): {e}")
...
def log_data_point(t, tgt, act, up, lo, viol):
    ...
    # Log only when the integer second changes (e.g., 1.0, 2.0, 3.0)
    if current_int_sec > last_logged_int_sec:
        row_data = [current_int_sec, f"{tgt:.1f}", f"{act:.1f}", f"{up:.1f}", f"{lo:.1f}", viol]
        row_str = ",".join(map(str, row_data)) + "\n"
        
        # 1. Save to Memory (For Download Button)
        last_log_content += row_str
        
        # 2. Save to Disk (For Backup)
    ...
    with pulse_lock:
        ...
        if delta_time > 0.01: 
            if last_pulse_time != 0:
                ...
                # This smooths out the "sharp edges"
                current_speed_kmh = (current_speed_kmh * 0.80) + (new_speed * 0.20)
                ...
                for row in reader:
                    try:
                        profile_data.append({
                            ...
                            'upper': float(row.get('upper', float(row['target'])+TOLERANCE_KMH)),
                            'lower': float(row.get('lower', max(0, float(row['target'])-TOLERANCE_KMH)))
                        })
                    except: pass
            else:
                f.seek(0)
...
async def loop():
    # 10Hz Loop Rate for Smoother UI
    while True:
