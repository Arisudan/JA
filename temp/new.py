#!/usr/bin/env python3
"""
backend.py - 5Hz Smooth Transition Version
"""
import asyncio
import websockets
import json
...
# Smoothness & Timing Settings
TICK_HZ = 5                # Updates per second (5Hz = 0.2s interval)
GRACE_SECONDS = 5.0        # Time before violations start counting
DEFAULT_TOL = 2.0          # Speed tolerance (+/- km/h)
...
def start_new_log():
    ...
    try:
        ...
        current_csv_writer.writerow(["# Report Generated by Hero Drive Cycle Aid"])
        current_csv_writer.writerow([f"# Start Time: {session_start_str}"])
    ...
    with pulse_lock:
        ...
        if delta_time > 0.01: 
            if last_pulse_time != 0:
                ...
                # HEAVY SMOOTHING (Low Pass Filter)
                # Keeps 85% of old speed, mixes in 15% of new reading.
                # This removes "sharp edges" from the data.
            ...
            if reader:
                for row in reader:
                    try:
                        ...
                        up = float(row.get('upper', float(row['target'])+DEFAULT_TOL))
                        lo = float(row.get('lower', max(0, float(row['target'])-DEFAULT_TOL)))
                        profile_data.append({'time':t, 'target':tgt, 'upper':up, 'lower':lo})
                    except: pass
            else:
                f.seek(0)
                for i, row in enumerate(csv.reader(f)):
                    if row: profile_data.append({'time': i*1.0, 'target': float(row[0]), 'upper': float(row[0])+DEFAULT_TOL, 'lower': max(0, float(row[0])-DEFAULT_TOL)})
...
async def loop():
    while True:
        ...
        # --- TICK RATE CONTROL ---
        # Sleep for 1/5 = 0.2 seconds to maintain 5Hz loop
...
async def main():
    ...
    load_profile()
    print(f"Server running on ws://0.0.0.0:{PORT} at {TICK_HZ}Hz")
