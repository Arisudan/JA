...
# If speed is too LOW, INCREASE this number.
# If speed is too HIGH, DECREASE this number.
# Standard 18-inch wheel + tire is approx 1.94m to 2.04m
WHEEL_CIRCUMFERENCE = 2.04  # Increased from 1.94 to fix the -2km/h error
...
def process_pulse():
    ...
    # Use monotonic clock for better precision preventing drift
    current_time = time.monotonic()
    
    with pulse_lock:
        delta_time = current_time - last_pulse_time
        
        # Debounce: ignore noise faster than 10ms
        if delta_time > 0.01: 
            if last_pulse_time != 0:
                speed_mps = WHEEL_CIRCUMFERENCE / delta_time
                new_speed = speed_mps * 3.6
                
                # Fast Smoothing: 20% old speed, 80% new speed (more responsive)
                current_speed_kmh = (current_speed_kmh * 0.2) + (new_speed * 0.8)
            
            last_pulse_time = current_time
            # Debug print to help verify speed
...
def setup_hardware(pin):
    ...
    try:
        ...
        try:
            ...
            GPIO.add_event_detect(pin, GPIO.FALLING, callback=gpio_callback, bouncetime=15)
...
def get_current_speed():
    ...
    # Timeout logic using monotonic time
    ...
    try:
        ...
        async for msg in ws:
            ...
            if cmd == 'start':
                ...
                # Use time.time() for absolute start time record, but elapsed uses monotonic diffs internally if needed
