#!/usr/bin/env python3
"""
backend.py - Instant Download & Flush Version
"""
import asyncio
import websockets
import json
...
def start_new_log():
    ...
    try:
        ...
        current_log_file.flush() # Force write to disk immediately
        print(f"[LOG] Started logging to {filename}")
        
        # Reset memory buffer for the download button
...
def log_data_point(t, tgt, act, up, lo, viol):
    ...
    if current_csv_writer and current_log_file:
        try:
            ...
            # --- CRITICAL FIX: FORCE SAVE TO DISK ---
            current_log_file.flush() 
            os.fsync(current_log_file.fileno()) 
            
            # Update the memory buffer so the Download button works instantly
...
def process_pulse():
    ...
    with pulse_lock:
        ...
        if delta_time > 0.01: 
            ...
            # print(f"[SENSOR] Pulse! Speed:{current_speed_kmh:.1f} km/h")
    ...
    try:
        ...
        async for msg in ws:
            ...
            if cmd == 'start':
                ...
                # Send whatever data we have so far immediately
                print("[CMD] Serving partial download...")
...
async def loop():
    while True:
        ...
        if test_state['running']:
            ...
            # Log Data Point (with Force Flush)
